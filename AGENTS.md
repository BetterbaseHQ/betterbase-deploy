...
Caddy publishes the only host ports: 80/443 plus `${ACCOUNTS_PORT:-5377}`/`${SYNC_PORT:-5379}`/`${EXAMPLES_PORT:-5380}` (the latter three are dead in TLS mode — nothing in the container listens on them). Everything else is internal.

## Commands

```bash
./setup.sh              # Create .env (random keys), generate OPAQUE keys, provision CAP
./setup.sh --domain d   # Same, but serves https://accounts.d + https://sync.d + https://examples.d (Let's Encrypt)
...
`.env.example` documents every variable. Required values are generated by `setup.sh`: `OPAQUE_SERVER_SETUP` (via the accounts image's `keygen` binary), `IDENTITY_HASH_KEY`, `CAP_ADMIN_KEY`, CAP site credentials (provisioned via CAP's admin API, using `jq`), and one `<APP>_CLIENT_ID` per example app (OAuth clients created via the accounts image's `oauth-client` binary — idempotent, reuses existing clients by name).
